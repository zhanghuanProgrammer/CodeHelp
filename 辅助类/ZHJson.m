...
- (void)changeMutableDicFromDictionary:(NSMutableDictionary *)dic forKey:(NSString *)key value:(NSString *)value{
    
    for (NSString *tempkey in dic) {
        id obj=dic[tempkey];
        if ([obj isKindOfClass:[NSMutableDictionary class]]) {
            [self changeMutableDicFromDictionary:obj forKey:tempkey value:value];
        }else if ([obj isKindOfClass:[NSMutableArray class]]){
            [self changeMutableArrFromArray:obj forKey:tempkey value:value];
        }
    }
...
- (void)changeMutableArrFromArray:(NSMutableArray *)arr forKey:(NSString *)key value:(NSString *)value{
    for (id obj in arr) {
        if ([obj isKindOfClass:[NSMutableDictionary class]]) {
            [self changeMutableDicFromDictionary:obj forKey:key value:value];
        }else if ([obj isKindOfClass:[NSMutableArray class]]){
            [self changeMutableArrFromArray:obj forKey:key value:value];
        }
    }
}


- (id)getValueFromDictionary:(NSDictionary *)dic forKey:(NSString *)key{
    ...
    for (NSString *tempkey in dic) {
        id obj=dic[tempkey];
        if ([obj isKindOfClass:[NSDictionary class]]) {
            id result=[self getValueFromDictionary:obj forKey:key];
            if(result) return result;
        }else if ([obj isKindOfClass:[NSArray class]]){
            id result= [self getValueFromArray:obj forKey:key];
            if(result) return result;
        }
    }
    return nil;
...
- (id)getValueFromArray:(NSArray *)arr forKey:(NSString *)key{
    for (id obj in arr) {
        if ([obj isKindOfClass:[NSDictionary class]]) {
            id result=[self getValueFromDictionary:obj forKey:key];
            if(result) return result;
