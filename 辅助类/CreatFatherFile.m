...
- (NSArray *)getArrFromText:(NSString *)text{
    if ([self judge:text]) {
...
- (void)backUp:(NSString *)fileName{
    ...
    NSString *filePathTemp=[NSHomeDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"Desktop/%@.m",fileName]];
    NSString *text=[NSString stringWithContentsOfFile:filePathTemp encoding:NSUTF8StringEncoding error:nil];
    
    NSString *filePath=[NSHomeDirectory() stringByAppendingPathComponent:@"Desktop/Log.m"];
...
- (NSString *)getInfoFromDic:(NSArray *)arr{
    ...
    for (NSInteger i=0; i<arr.count; i++) {
        if([arr[i] hasSuffix:@"%@"]){
            NSString *strTemp=arr[i];
            strTemp = [strTemp substringToIndex:strTemp.length-2];
            if (i!=arr.count-1) {
                [self insertValueAndNewlines:@[[NSString stringWithFormat:@"\"%@\":\"<#%@#>\",",strTemp,strTemp]] ToStrM:strM];
            }else{
                [self insertValueAndNewlines:@[[NSString stringWithFormat:@"\"%@\":\"<#%@#>\"",strTemp,strTemp]] ToStrM:strM];
            }
        }else{
