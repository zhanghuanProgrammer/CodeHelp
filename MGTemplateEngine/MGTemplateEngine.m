...
- (id)resolveVariableWithMark:(NSString *)var{
    id parent = nil;
    NSString *key = nil;
    while ([var rangeOfString:@"{{ "].location!=NSNotFound) {
        var=[var stringByReplacingOccurrencesOfString:@"{{ " withString:@"{{"];
    }
    while ([var rangeOfString:@" }}"].location!=NSNotFound) {
        var=[var stringByReplacingOccurrencesOfString:@" }}" withString:@"}}"];
    }
    NSArray *markContents=[ZHNSString getMidStringBetweenLeftString:@"{{" RightString:@"}}" withText:var getOne:NO withIndexStart:0 stopString:nil];
    
    if(markContents.count>0){
        for (NSString *markContent in markContents) {
            id result=[self resolveVariable:markContent];
            if(result && [result isKindOfClass:[NSString class]]){
                var=[var stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@"%@%@%@",@"{{",markContent,@"}}"] withString:result];
            }
        }
    }
    id result = [self valueForVariable:var parent:&parent parentKey:&key];
