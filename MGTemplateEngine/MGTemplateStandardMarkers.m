			   ...
			   currentBlockInfo:(NSDictionary *)blockInfo newVariables:(NSDictionary * __autoreleasing *)newVariables
{
	if ([marker isEqualToString:FOR_START]) {
		if (args && [args count] >= 3) {
            ...
            if([endArg isEqualToString:@"{{"]&&args.count>4){
                endArg=[NSString stringWithFormat:@"%@ %@ %@",[args objectAtIndex:2],[args objectAtIndex:3],[args objectAtIndex:4]];
            }
			...
			if (isRange) {
				...
				startIndex = 0;
                NSObject *obj=nil;
                if([endArg rangeOfString:@"{{"].location!=NSNotFound){
                    obj = [engine resolveVariableWithMark:endArg];
                }else{
                    obj = [engine resolveVariable:endArg];
                }
