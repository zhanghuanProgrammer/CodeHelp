...
+ (NSString *)changeFileOldPath:(NSString *)oldPath newFileName:(NSString *)newName{
    ...
    if (pathExtension.length>0&&[newName hasSuffix:[NSString stringWithFormat:@".%@",pathExtension]]==NO) {
        newPath=[newPath stringByAppendingString:[NSString stringWithFormat:@".%@",pathExtension]];
    }
    [self moveItemAtPath:oldPath toPath:newPath];
    return newPath;
}
...
+ (void)creatTargetDirectorIfNotExsit:(NSString *)DirectorPath{
    NSString *exsitDirector=DirectorPath;
    if (![ZHFileManager fileExistsAtPath:exsitDirector]) {
        exsitDirector=[ZHFileManager getUpLeverDirector:exsitDirector];
        if (![ZHFileManager fileExistsAtPath:exsitDirector]) {
            [self creatTargetDirectorIfNotExsit:exsitDirector];
        }
        [ZHFileManager creatDirectorIfNotExsit:DirectorPath];
    }
}
...
+ (void)subPathDirectorsArrInDirector:(NSString *)DirectorPath saveToPathDirectorsArr:(NSMutableArray *)pathDirectorsArr{
    NSArray *directorArr=[self subPathDirectorArrInDirector:DirectorPath];
    for (NSString *filePath in directorArr) {
        if (![pathDirectorsArr containsObject:filePath]) {
            [pathDirectorsArr addObject:filePath];
            [self subPathDirectorsArrInDirector:filePath saveToPathDirectorsArr:pathDirectorsArr];
        }
    }
}
...
+ (NSArray<NSString *> *)contentsOfDirectoryAtPath:(NSString *)path{
    ...
    NSMutableArray *pathFileArr=[NSMutableArray array];
    for (NSString *str in filesArr) {
        if (str.length>0) {
            [pathFileArr addObject:[path stringByAppendingPathComponent:str]];
        }
    }
    if (pathFileArr.count>0) {
        return pathFileArr;
...
+ (NSString *)creatCurDataDirectorToMacDestop{
    NSString *filePath=[ZHFileManager getDesktopInMac];
